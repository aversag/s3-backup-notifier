...
deploy:
	aws cloudformation deploy \
			...
			--parameter-overrides \
				ENV=${ENV} \
				MONITORINGBUCKET=s3monitoring-artifacts \
				S3PREFIX=${S3PREFIX} \
				PROJECT=${PROJECT} \
				BLACKLISTEDBUCKETS=${BUCKETSBLACKLIST} \
				SLACKWEBHOOKURL=${SLACK_WEBHOOK_URL}

layer: clean-layer
	pip3 install requests -t python/
	zip -r layer.zip python/
	aws lambda publish-layer-version --layer-name boto3 --zip-file fileb://layer.zip --compatible-runtimes python3.9
